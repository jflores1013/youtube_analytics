...
def main(df):
  ...
  table = {'channel_id': channel_id_list, 'date': date_list, 'estimated_minutes_watched': emw_list, 'views': views_list, 'likes': likes_list, 'subscribers_gained': subscribers_gained_list, 'subscribers_lost': subscribers_lost_list, 'average_view_duration': average_view_list, 'shares': shares_list, 'dislikes': dislikes_list}
  df = pd.DataFrame(table, columns=['channel_id', 'date', 'estimated_minutes_watched', 'views', 'likes', 'subscribers_gained', 'subscribers_lost', 'average_view_duration', 'shares', 'dislikes'])
  
  df = df.astype({'channel_id': 'str', 'date': 'str', 'estimated_minutes_watched': 'int', 'views': 'int', 'likes': 'int', 'subscribers_gained': 'int', 'subscribers_lost': 'int', 'average_view_duration': 'int', 'shares': 'int', 'dislikes': 'int'})

  return df

main(df).to_csv('REPLACE_WITH_YOUR_OUTPUT_LOCATION/youtube_analytics_channel.csv')

def get_output_schema():
   return pd.DataFrame({
       'channel_id': prep_string(),
       'date': prep_string(),
